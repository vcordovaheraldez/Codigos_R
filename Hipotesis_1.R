...
# Hipótesis 1 
  # Indicador 2: Caudal territorial de las PTAR con un régimen privado (2017-2021)
...
# Porcentaje de cada categoría poblacional del total de municipios en México categorizados manualmente 
...
# Reenombar variables
inafed_municipios = dplyr::rename(inafed_municipios, clave_mun = cve_inegi)

# Unión de ambas bases 
base_unida <- left_join(inafed_municipios, pobmun, by = "clave_mun")

# Descargar 
write.csv(base_unida, file = "Poblacion_2020.csv", row.names = FALSE)

# Calcular cantidad y porcentaje por categoría
porcentaje <- base_unida %>%
  group_by(categoria_poblacion) %>%
  summarise(Cantidad = n()) %>%
  mutate(Porcentaje = round((Cantidad / sum(Cantidad)) * 100, 2))

# Filtrar categorías de interés
porcentaje <- porcentaje %>%
  filter(categoria_poblacion %in% c("Metropolitanos", "Urbanos", "Semiurbanos", "Rurales"))

print(porcentaje)

# Datos totales por categoría
totales <- c(Rurales = 674, Semiurbanos = 1349, Urbanos = 362, Metropolitanos = 84)

# Dataframe con datos de cada Censo Nacional (municipios con al menos una PTAR)
ptar <- data.frame(
  Año = c(2017, 2017, 2017, 2017,
          2019, 2019, 2019, 2019,
          2021, 2021, 2021, 2021),
  Categoria = rep(c("Rurales", "Semiurbanos", "Urbanos", "Metropolitanos"), 3),
  Cantidad = c(29, 146, 108, 43,
               102, 390, 220, 75,
               118, 371, 173, 61)
)
...
ptar <- ptar %>%
  mutate(Total = totales[Categoria],
...
# Porcentaje de cada nivel de desarrollo económico del total de municipios en México categorizados manualmente
...
# Reenombar variables
inafed_municipios = dplyr::rename(inafed_municipios, plnt_folio = clave_mun)

# Unión de ambas bases 
base_unida1 <- left_join(inafed_municipios, IDH_2015, by = "plnt_folio")

# Descargar 
write.csv(base_unida1, file = "NivelDesarrollo_2020.csv", row.names = FALSE)

# Calcular cantidad y porcentaje por nivel
resultado <- base_unida1 %>%
  group_by(gdh) %>%
  summarise(Cantidad = n()) %>%
  mutate(Porcentaje = round((Cantidad / sum(Cantidad)) * 100, 2))
...
# Dataframe con las cantidades de nivel de desarrollo
datos <- data.frame(
  Año = c(2017, 2017, 2017, 2017,
          2019, 2019, 2019, 2019,
          2021, 2021, 2021, 2021),
  Nivel = rep(c("Bajo", "Medio", "Alto", "Muy alto"), 3),
  Cantidad = c(11, 162, 120, 33,
               32, 418, 292, 45,
               42, 401, 240, 40)
)

# Totales por nivel de desarrollo (según la base original)
totales <- c(Bajo = 260, Medio = 1552, Alto = 583, `Muy alto` = 61)

# Agregar columna de porcentaje al dataframe existente
datos <- datos %>%
  mutate(Porcentaje = round((Cantidad / totales[Nivel]) * 100, 2))
