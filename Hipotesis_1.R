...
# Gráfico
  geom_label(
    ...
    aes(label = paste0(round(porcentaje_privado, 1), "%")),
...
# Filtrar NA 
Inventario_Mexico_con_Municipio <- Inventario_Mexico_con_Municipio %>%
  filter(!is.na(cau_trat))

# Transformar caudal a numérico
Inventario_Mexico_con_Municipio$cau_trat <- as.numeric(Inventario_Mexico_con_Municipio$cau_trat)

# Suma caudal tratado
suma_caudal <- sum(Inventario_Mexico_con_Municipio$cau_trat, na.rm = TRUE)

print(suma_caudal)

# Unión de ambas bases 
datos_combinados <- left_join(Inventario_Mexico_con_Municipio,IDH_2015,by = c("Estado", "Municipio"))

# Suma caudal tratado
suma_caudal <- sum(datos_combinados$cau_trat, na.rm = TRUE)
...
# Unión de ambas bases 
Inventario_categorias <- left_join(datos_combinados, pobmun, by = "plnt_folio")

# Suma caudal tratado
suma_caudal <- sum(Inventario_categorias$cau_trat, na.rm = TRUE)

print(suma_caudal)

# Filtrar NA
Inventario_sin_NA <- Inventario_categorias %>%
  filter(!is.na(tipo_regimen),
         !is.na(categoria_poblacion),
         !is.na(gdh))

# Agrupar por el tipo de regimen 
resumen_regimen <- Inventario_sin_NA %>%
  group_by(tipo_regimen) %>%
  summarise(caudal_categorizado = sum(cau_trat, na.rm = TRUE))

print(resumen_regimen)

# Agrupar por el tipo de clasificación poblacional
resumen_poblacion <- Inventario_sin_NA %>%
  group_by(categoria_poblacion) %>%
  summarise(caudal_categorizado = sum(cau_trat, na.rm = TRUE))

print(resumen_poblacion)

# Agrupar por el desarrollo económico
resumen_gdh <- Inventario_sin_NA %>%
  group_by(gdh) %>%
  summarise(caudal_categorizado = sum(cau_trat, na.rm = TRUE))

print(resumen_gdh)

# Agrupar por todas las categorías  
resumen_categorias <- Inventario_sin_NA %>%
  group_by(tipo_regimen, gdh, categoria_poblacion) %>%
  summarise(caudal_categorizado = sum(cau_trat, na.rm = TRUE))

print(resumen_categorias)

# Descargar 
write.csv(resumen_categorias, file = "Resumen_categorias.csv", row.names = FALSE)

# Porcentaje privado
resumen_categorias <- resumen_categorias %>%
  group_by(gdh, categoria_poblacion) %>%
  mutate(porcentaje = caudal_categorizado / sum(caudal_categorizado) * 100)

# Gráfico
ggplot(resumen_categorias, aes(x = categoria_poblacion,
                               y = caudal_categorizado,
                               fill = tipo_regimen)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  
  # Solo etiquetas para régimen privado
  geom_label(data = subset(resumen_categorias, tipo_regimen == "Privado"),
  ...
  facet_wrap(~ gdh) +
  scale_fill_manual(values = c("#1f3552", "#6b8ea1")) +
  theme_minimal() +
  theme(
