...
# Hipótesis 2
  # Indicador: Los diferentes reúsos del agua tratada (2022)
...
resumen_reuso <- Reuso_clasificado_manual %>%
  ...
  summarise(
    Numero_de_Plantas = n(),
    Suma_Caudal = sum(Caudal),
    .groups = "drop"
  )

# Gráfico
ggplot(resumen_reuso, aes(
  x = categoria_poblacion,
  y = Suma_Caudal,
  fill = tipo_regimen
)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  facet_wrap(~ fct_relevel(gdh, "MUY ALTO", "ALTO", "MEDIO")) +
  scale_fill_manual(values = c("brown", "steelblue")) +
  labs(
    title = "Caudal destinado al reúso por régimen, desarrollo económico y población",
    x = "Categoría poblacional",
    y = "Caudal total destinado al reúso",
    fill = "Régimen jurídico"
  ) +
