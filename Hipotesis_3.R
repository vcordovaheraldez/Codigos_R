...
# Modelos con interacción por tipo de tratamiento 

# Dataframe con Primario y Primario mejorado
df_primario <- Base_2019m %>%
  filter(nivel_tratamiento %in% c("Primario", "Primario mejorado"))

# variable de referencia 
df_primario$tipo_regimen <- relevel(df_primario$tipo_regimen, ref = "Público")
df_primario$gdh <- relevel(df_primario$gdh, ref = "MUY ALTO")
df_primario$categoria_poblacion <- relevel(df_primario$categoria_poblacion, ref = "Metropolitanos")

# Modelo con interacción
modelointer_primario=lm(log(tra_peso)~log(vol_trat)+tipo_regimen*tr_po_pl_a+categoria_poblacion+gdh, data=df_primario)
summary(modelointer_primario)

# Dataframe con Secundario
df_secundario <- Base_2019m %>%
  filter(nivel_tratamiento == "Secundario")

# variable de referencia 
df_secundario$tipo_regimen <- relevel(df_secundario$tipo_regimen, ref = "Público")
df_secundario$gdh <- relevel(df_secundario$gdh, ref = "MUY ALTO")
df_secundario$categoria_poblacion <- relevel(df_secundario$categoria_poblacion, ref = "Metropolitanos")

# Modelo con interacción
modelointer_secundario=lm(log(tra_peso)~log(vol_trat)+tipo_regimen*tr_po_pl_a+categoria_poblacion+gdh, data=df_secundario)
summary(modelointer_secundario)

# Dataframe con Secundario/Terciario y Terciario
df_terciario <- Base_2019m %>%
  filter(nivel_tratamiento %in% c("Secundario/Terciario", "Terciario", "Otro"))

# variable de referencia 
df_terciario$tipo_regimen <- relevel(df_terciario$tipo_regimen, ref = "Público")
df_terciario$gdh <- relevel(df_terciario$gdh, ref = "MUY ALTO")
df_terciario$categoria_poblacion <- relevel(df_terciario$categoria_poblacion, ref = "Metropolitanos")

# Modelo con interacción
modelointer_terciario=lm(log(tra_peso)~log(vol_trat)+tipo_regimen*tr_po_pl_a+categoria_poblacion+gdh, data=df_terciario)
summary(modelointer_terciario)

# Extraer coeficientes y convertir a data frame
df_primario <- tidy(modelointer_primario) %>% mutate(Nivel = "Primario")
df_secundario <- tidy(modelointer_secundario) %>% mutate(Nivel = "Secundario")
df_terciario <- tidy(modelointer_terciario) %>% mutate(Nivel = "Terciario")

# Unir todos
df_comparativo <- rbind(df_primario, df_secundario, df_terciario)

# Filtrar solo términos relevantes (tipo_regimenPrivado y sus interacciones)
df_comparativo <- df_comparativo %>%
  filter(grepl("tipo_regimenPrivado", term))

# Gráfico
ggplot(df_comparativo, aes(x = Nivel, y = estimate, fill = Nivel)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_errorbar(aes(ymin = estimate - std.error, ymax = estimate + std.error),
                width = 0.2, position = position_dodge(0.9)) +
  labs(title = "Comparación del efecto del régimen privado en costos energéticos",
       x = "Nivel de tratamiento",
       y = "Coeficiente estimado") +
  scale_fill_manual(values = c("Primario" = "darkgreen", 
                               "Secundario" = "brown", 
                               "Terciario" = "steelblue")) +
