...
# Modelo 
modelo5=lm(log(tra_peso)~log(vol_trat)+nivel_tratamiento*tipo_regimen+categoria_poblacion, data=Base_2019m)
summary(modelo5)
...
# Tecnología más común del régimen  privado (Lodos activados)

table(Base_2019m$tr_po_pl_a, Base_2019m$tipo_regimen)
table(Base_2019m$nivel_tratamiento, Base_2019m$tipo_regimen)

# Dataframe con tecnología de lodos activados
df_lodos <- Base_2019m %>%
  filter(tr_po_pl_a %in% c("Lodos activados"))

# Variables de referencia 
df_lodos$tipo_regimen <- relevel(df_lodos$tipo_regimen, ref = "Público")
df_lodos$gdh <- relevel(df_lodos$gdh, ref = "MUY ALTO")
df_lodos$categoria_poblacion <- relevel(df_lodos$categoria_poblacion, ref = "Metropolitanos")

# Modelo 
modelo_lodos=lm(log(tra_peso)~log(vol_trat)+tipo_regimen+gdh+categoria_poblacion, data=df_lodos)
summary(modelo_lodos)
